function [B,Wstar,T,P,Q,W,R2X,R2Y, RMSE]=plsnipals(S,Y, vl) %
% The NIPALS (Non-linear iterative partial least squares) algorithm 
% for PLS with one or more Y
% 
% Inputs Arguments:
%       S: Spectra (n x p matrix)
%       Y: Responses (n x m matrix)
...
if vl<2; % If the user do not choose the number of latents variables
    ...
    while abs(R2newX-R2oldX)>0.01&&abs(R2newY-R2oldY)>0.01||i<3
        R2oldX=R2newX;
        R2oldY=R2newY;
