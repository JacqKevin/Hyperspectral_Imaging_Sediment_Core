function [C,label,clrmap,Rv_s, p_s, Ss, Yc] = IndiceCalculation_CorrRef(M,RGB,d,wl,indx,norm,fig,Y,dY,pas,pas2)
...
if sum(wl<800&wl>600)>0
    ...
    if sum(double(strcmp('Chlorophylls RABD670',index_vnir_selec)))
        ...
        IM=(((ind2(1)-ind3(1))*squeeze(M(:,:,ind1(1)))+...
            ...
            (ind2(1)-ind1(1)))./squeeze(M(:,:,ind3(1)));
        [a,b]=find(IM==inf);
        for j=1:length(a)
            IM(a(j),b(j))=NaN;
        end
    ...
    if nargin>7
        Rv_s=zeros(size(C,3),size(Y,2));
        p_s=zeros(size(C,3),size(Y,2));
        Ss=zeros(size(C,3),size(Y,2));
        for j=1:size(Y,2)
            for i=1:size(C,3)
                [Rv_s(i,j), p_s(i,j), Ss(i,j), Yc{i,j}] = CorrelIndiceMap(squeeze(C(:,:,i)),d,Y(:,j),dY,0,pas,pas2);
            end
        end
    end
...
if sum(wl<2400&wl>1100)>0
    ...
    if sum(double(strcmp('Illite-muscovite Crystallinity R1900/R2200',index_swir_selec)))
        ...
        [~,ind1]=find(abs(wl-wlr(1,1))==min(abs(wl-wlr(1,1))));
        [~,ind2]=find(abs(wl-wlr(1,2))==min(abs(wl-wlr(1,2))));
        IM=(squeeze(M(:,:,ind1(1)))./Rmean)./(squeeze(M(:,:,ind2(1)))./Rmean);
        [a,b]=find(IM==inf);
        for j=1:length(a)
            IM(a(j),b(j))=NaN;
        end
